...
- **FWHM 参考**（`--fwhm`）：查某个元素/化合物的实测 FWHM 统计，用于设 `sigma_range`。仅 12.6% 记录有 FWHM，数据稀疏时以 `peak-fitting.md` 的 FWHM 典型范围表为准。
- **自旋-轨道分裂**（`--split`）：查 p/d/f 轨道的双峰分裂值。⚠️ NIST 两个 j 分量的记录数严重不平衡，推算值仅供参考——**以 `peak-fitting.md` 中的物理常数表为准**。
