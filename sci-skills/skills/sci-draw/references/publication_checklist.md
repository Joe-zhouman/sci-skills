...
- [ ] 数据图（线/柱/散点/热力/箱线）→ **矢量 PDF**（首选）
- [ ] 显微图、照片 → **PNG**（≥300 DPI）
- [ ] **没有 JPEG**（数据图禁用）
- [ ] PDF 嵌入 TrueType 字体（**fonttype 42**），不含 Type 3
- [ ] 用 `export_figure(formats=['pdf', 'png'])` 导出
...
n 定义:              （生物重复 / 技术重复）
中心统计量:          （均值 / 中位数）
离散区间:            （SD / SEM / 95% CI / IQR）
检验方法:            （t-test / Mann-Whitney / ANOVA）
多重比较校正:        （Bonferroni / FDR / 无）
p 值呈现:            （精确值 / 符号 * p<0.05）
源数据文件:          （CSV/TSV 路径）
```
...
全局调整通常比局部选择性编辑更安全。如果调整改变了背景或条带的可见性，标记出来。

## 图型契约 & 数据溯源

- [ ] 核心结论已写成一句话（这张图要论证什么）
- [ ] 每个 panel 都映射到核心结论（无多余 panel）
- [ ] 图型原型已声明（quantitative grid / schematic-led composite / image plate / asymmetric mixed-modality）
- [ ] 定量 panel 可追溯到干净的 CSV/TSV/XLSX 或脚本输出（source data traceability）
- [ ] 显微/印迹图有 scale bar、标定、raw file provenance
