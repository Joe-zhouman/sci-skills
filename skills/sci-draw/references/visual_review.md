...
```
绘制 → ① 程序自检(visual_qa) → ② 导出 PDF+PNG → ③ 人眼复核
                                                      ↓ 发现问题
        ⑤ 通过 ← ④ 回到对应步骤改图 → 重新导出 → 再检查 ←┘
```
...
| 人眼复核 | 打开导出的 PNG/PDF，逐条对照清单 | 图例压数据、标注重叠、子图标签对齐、配色/灰度可分、整体观感（感知性） |

**两层都要过。** 程序 PASS 不代表图就好看；人眼复核抓的是程序难判的感知问题。

论文 figure 不会很多（通常 4-8 张），人眼看一遍比任何自动化都可靠。
...
- 所有模型都能完成这一步——多模态用 `Read`，非多模态用 `vision-bridge` 桥接。
...
打开导出的 PNG 或 PDF，**逐条**对照下面的清单核对。不要扫一眼说"看起来不错"——一项一项看。
