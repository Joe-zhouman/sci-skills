...
**Never** use Type 3 fonts in PDF (matplotlib default): set `pdf.fonttype = 42` before
any `savefig` call. The mandatory rcParams lines (see api.md) must be set first.
...
- [ ] **MANDATORY first lines**: `font.family='sans-serif'`, `font.sans-serif=['Arial','DejaVu Sans','Liberation Sans']`, `pdf.fonttype=42`, `svg.fonttype='none'`
- [ ] **Export via** `export_figure(formats=['pdf', 'png'])`. PDF is primary vector format.
