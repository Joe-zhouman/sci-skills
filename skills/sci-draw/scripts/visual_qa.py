...
def _demo() -> int:
    ...
    print("=== visual_qa demo：对一张刻意做坏版面的图自检 ===", file=sys.stderr)
    issues = audit_layout(fig)
    print_report(issues)
    out = render_preview(fig, "./visual_qa_demo.png", dpi=120)
    print(f"\n预览已写出：{out}（可用 Read 工具读图复核）", file=sys.stderr)
    print("注：缺字检测只在中文未配字体等情况下才会 FAIL；本 demo 主要展示裁切+重叠。", file=sys.stderr)
    return 0


def _cli() -> int:
    p = argparse.ArgumentParser(description="sci-draw visual QA")
    ...
    if args.preview:
        ...
        if args.format == "json":
            json.dump({"preview": out}, sys.stdout, ensure_ascii=False)
            sys.stdout.write("\n")
        else:
            print(f"[visual_qa] 预览：{out}", file=sys.stderr)
    else:
        msg = ("对已落盘图片只支持 --preview 渲图；"
