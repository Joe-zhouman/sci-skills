...
def _cli() -> int:
    p = argparse.ArgumentParser(description="sci-draw style setup")
    ...
    if args.list_fonts:
        ...
        if args.format == "json":
            json.dump({"cjk_fonts": fonts}, sys.stdout, ensure_ascii=False, indent=2)
            sys.stdout.write("\n")
        else:
            if fonts:
    ...
    if args.test:
        ...
        keys = ("figure.figsize", "font.family", "font.size",
                "axes.labelsize", "pdf.fonttype", "axes.unicode_minus")
        if args.format == "json":
            result = {**info, **{k: plt.rcParams[k] for k in keys}}
