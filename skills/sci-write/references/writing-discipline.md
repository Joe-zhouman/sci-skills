...
以下节点不能只做技术验证——资深学术工作者的判断无法被自动化替代。**主动停下来，让 user 用自己的经验审。**

| 节点 | 技术验证能做什么 | 人类必须判断什么 |
|---|---|---|
| **claim 确认** (Step 0) | 数据能撑住 claim 吗？对标文献的竞争水平？ | 这个 claim 在本领域同行眼里够不够分量？审稿人会买账吗？ |
| **paper-plan 确认** (Step 1) | 每张图有 conclusion + claim 吗？ | 图够吗？多了还是少了？有没有该做但没列的图？ |
| **图义核查** (Step 3) | 图显示的内容和 conclusion 一致吗？有没有视觉误读风险？ | 统计方法合适吗？同一组数据，不同专业背景的人会不会读出不同结论？这个 conclusion 是从数据里直接推导的，还是推了太多步？ |
| **正文 draft** (Steps 4-5) | claim-evidence 对得上、术语一致、动词校准 | 这句话的措辞在你的领域里是不是说得太重/太轻了？你的同行会怎么解读这段？ |

技术验证跑完 → **不要直接进下一步。** 把结果给 user 看，问一句："你的经验怎么判断？" User 的学术直觉是最后一道闸。

尤其图义核查：视觉读图只能告诉你"图显示的是什么"，不能告诉你"这个 conclusion 是否合理、统计方法是否合适、同行会不会有不同解读"。这些问题只有人回答。
