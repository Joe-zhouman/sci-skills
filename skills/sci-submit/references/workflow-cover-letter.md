...
| 需要的内容 | 从哪里取 |
|---|---|
| Cheat Sheet（背景钩子要点 + 核心发现要点） | `manuscript-meta.md` → Cover Letter Cheat Sheet |
...
Cheat Sheet → 核心发现。用户的要点可能包含定量数据——转成定性表述（"差两倍"不是 "p=0.003"）。一句顶过的独特点放在段尾。
...
从 `hard-constraints.md` 反推该对谁说，翻译成学术语言。"我要毕业" → "addresses a timely gap..."。"导师要求投" → 不体现在这一段。
...
路径：`sci-submit/cover-letter/<journal-name>-<YYYY-MM-DD>/cover-letter.tex`（或 `.docx`）
...
4. **反馈 → 改 → 再截。** 循环到没问题。
5. **画完导出。** TIFF/EPS/PDF/PPTX，300dpi。放 `sci-submit/cover-letter/<journal-name>-<YYYY-MM-DD>/toc-figure.tiff`。
...
封面信写好了，GTOC（如需要）画好了，元数据填满了。现在打开浏览器。

**告诉用户**：

> "好，准备好了。封面信在 `sci-submit/cover-letter/<journal-name>-<YYYY-MM-DD>/`，GTOC 也在那。元数据都在 `sci-submit/manuscript-meta.md` 里。打开 [期刊名] 的投稿页面，我告诉你在每个页面贴什么。"
...
> "上传或粘贴封面信。文件：`sci-submit/cover-letter/<journal-name>-<YYYY-MM-DD>/cover-letter.pdf` 或 `.docx`。如果只有文本框，把内容贴进去。"
