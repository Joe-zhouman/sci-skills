...
## 为什么这个 skill 是家族的核心卖点

表面看它是"搭目录的快捷入口"。实际它是**整个家族解耦战略的兑现机**——

1. **它把契约从理念变成实物。** 跑一次 `init`，用户立刻拿到三份 `.README.md`（每个子目录一份），白纸黑字写着"放什么、schema、谁读"。契约不再是文档描述，是落地文件。show, don't tell。
2. **它是零成本试用入口。** 观望用户不必先信我们的 sci-write、不必抛弃自己的画图工具——跑 init 拿到"插槽"（目录契约），就能先用别人的零件、再换我们的零件。插槽先铺好，零件慢慢换。这是降低采用门槛，而降门槛是解耦战略的生存关键。
3. **它是迁移友好性的载体。** `migrate` 模式让老布局项目一条命令迁过来，把"解耦对老用户也好"从理论变成实测。
4. **它是契约的单一权威源。** 契约写在 init 脚本里、由 init 生成为文件——改契约改 init，下次跑就刷新。不会散在各 skill 文档里不一致。

一个想了解本家族的人，跑一次 init 就理解全部精髓。它是家族对外展示解耦价值的第一触点。

（战略背景：科研 skills 群雄割据，每人有自己喜好。紧耦合全家桶逼用户 all-or-nothing，
结果谁都用不上。本家族走解耦——每个 skill 是可替换零件，靠目录契约咬合，能跟别人造的
零件配合。详见 glossary "解耦是生存策略" 条与 `references/family-layout.md`。）
