...
1. **near-trivial consistency gate** — `scripts/check_intro.py` (the gate) +
   `scripts/test_check_intro.py` (23 stdlib cases, run `python3 test_check_intro.py`).
   Exit-code contract: 0 = consistency through; 1 = consistency issues (each printed).
   Cases covered:
   - passes on a settled gap-map.md (with `intro-tex: chapter0.tex`) + its chapter-map.md
     + the named intro tex file (2 gaps, all fields filled, status=filled, filled-by
     chapters resolve);
   ...
   - fails on a missing `intro-tex` file (the `intro-tex` field names a file absent
     from thesis/tex/ — intro didn't produce its tex; aries #2);
   - fails on a missing `intro-tex` field (gap-map.md has no top-level
     `intro-tex:` — aries #2: template-derived filename, not hardcoded);
   - passes with a template-named intro tex (`chapter0.tex` per generic-test
     template-spec — aries #2: the gate is template-agnostic, not hardcoded to
     `ch0-intro.tex`);
   ...
   - fails on a malformed filled-by (`some chapter` — no parseable chapter number,
     can't cross-ref);
   - fails on a multi-chapter filled-by (`Chapter 1 Chapter 999` — two numbers;
     spec: one gap→one chapter; aries #5);
   - fails on an unreadable chapter-map.md (binary/permission-denied — the gate
     appends a "cross-ref 跳过" issue + exit 1, NOT silent swallow; taurus fix /
     aries #3);
   ...
   - **accepts gap headers with a trailing title** — `## Gap 1 (研究背景)` parses
     ...
     trailing-title test);
   - **ignores a UTF-8 BOM** — a gap-map.md starting directly with `## Gap 1` + a BOM
     (Windows editor) must not drop Gap 1 from checks; its dangling filled-by is still
     caught (aries #1: both reads use `utf-8-sig`).
...
2. **the split (spec §⑥, stated honestly)** — check_intro.py is **NEAR-TRIVIAL
   ...
   missing intro-tex file). It does **NOT** catch depth — a gap no chapter genuinely
   ...
   `gap`/`filled-by`/`callback-anchor` field presence + value, `status` keyword,
   `filled-by` chapter number ∈ chapter-map.md's `## Chapter N` set, `intro-tex`
   file existence) — mirroring spine/init/dissect's justified deviation
   (deterministic code + verifiable outputs).
...
4. **decoupling assertions (programmatic)** —
   ...
   - intro writes `thesis/tex/<intro-tex>` (the template-named intro file, per
     template-spec.md — NOT hardcoded) + `sci-skills/thesis-intro/gap-map.md`
...
+ `status` + `filled-by` cross-ref + `intro-tex` file existence, not `anchor-in-intro`).
