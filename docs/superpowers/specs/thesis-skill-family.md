...
3. **正文不能照搬小论文的 IMRaD。** 学位论文正文不是 intro→method→results→discussion 的写法，而是 method 紧跟对应 results 的模块化组织（按模块分，每个模块有自己的 method 和 results）。现有工具没有这个重构能力——它们要么生成 IMRaD，要么照搬小论文结构。
...
article 家族（sci-write/sci-story/sci-export/sci-polish/sci-typeset）是为单篇期刊论文设计的：manuscript 按**审稿轮次**组织、your-paper-your-way 投稿时才搬模板、单篇 IMRaD、单篇的 Intro-Discussion coherence。学位论文本质不同：按**章**组织、模板写前定死、多论文整合、模块化正文、绪论↔总结的 thesis 级闭环。硬塞进 article 家族会破坏其单篇契约。thesis 需要自己的家族——但**镜像** article 的罗盘文件耦合哲学，复用其成熟的单篇机制升尺度。
...
#### ② 6 skill + 1 init，不是单 orchestrator

痛点"主线没贯通就往下写，写到总结才发现 callback 不起来 → 全盘返工"的解法是：把写作链拆成独立 skill，每个产落盘文件，状态在盘上跨 session 存活，可单独回退。

写作链：`thesis-spine`（提主线+统一框架）→ `thesis-dissect`（拆小论文写正文章，拆即写）→ `thesis-intro`（写绪论）→ `thesis-summary`（写总结，callback+共性）→ `thesis-theory`（写共用理论方法，放最后）。后处理：`thesis-typeset` + `thesis-polish`。
...
痛点 3（正文不能照搬 IMRaD，要模块化重构）的解法：dissect 拆小论文为章时**当场写正文章**。因为拆的时候逻辑已经捋清，分开（先拆后写）要重新载入一遍捋过的逻辑，是浪费。tex 单章单文件让"拆即写"落地干净。模块化重构（IMRaD→method-results 对）的章内纪律（question→method→results 三元）由 dissect 自带 reference。
...
**Cheap-regime 的诚实取舍**（aquarius #6）：glossary 三入口架构说 thick-orchestration entry 是 cheap-model regime 的永久需求。thesis 家族 v1 只有 6 execution + 1 init，**没有 orchestration entry**——cheap-model 用户得手动串 6 个 skill。这是**deliberate v1 cut**：v1 只服务能自己串的强模型用户；thick-orchestration entry（`using-thesis-skills`）是 future work，不在 v1。明确声明，不混"无单 orchestrator skill"和"无 orchestration entry"。
...
- **复用 article 家族的 sci-export/sci-polish/sci-typeset**：拒绝。学位论文的格式和润色与 article 差异大，硬塞会让两者都臃肿且侵入 article 既有契约。镜像其罗盘文件耦合哲学，但重搞。
...
- **有文件夹**（存 working notes，正文 tex 仍写进 `thesis/tex/`）：`thesis-dissect`（带每篇小论文第3级）/ `thesis-intro` / `thesis-theory` / `thesis-summary`。
...
| `thesis-terminology-ledger.md` | spine 建 | 各章/polish 共写 | 跨章术语统一 |
...
- **thesis-dissect**（拆小论文写正文章，**拆即写**）：读 `thesis-spine.md`+registry+template-spec；逐篇拆+当场写 `thesis/tex/chN.tex`（文件名按模板）；模块化重构 IMRaD→method-results 对，question→method→results 三元；章间递进落地。**默认 1:1 一篇一章（paper-X 文件夹），但 chapter-map 支持非 1:1**（合并多篇为一章、拆一篇为多章——aquarius #8 补，paper-X 是拆解笔记单位不强制等于章）。产正文章+`thesis-dissect/paper-X/`笔记+回填 `chapter-map.md`。**门**：每章答得出实例化框架+递进依赖（coverage 机械可查）；答不上 fallback spine。
...
3. **正文模块化重构**：dissect 把 IMRaD 重构成 method-results 对。**验收**：每个正文章 method 紧跟对应 results（非 IMRaD），每模块 question→method→results 三元。
