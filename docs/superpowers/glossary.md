...
**Intro↔Summary coherence lock**:
The thesis-scale escalation of sci-story's within-skill Intro-Discussion coherence: every narrative gap the 绪论 articulates must be callback'd by the 总结展望 chapter. Because thesis chapters are written cross-skill and cross-session, the lock splits into two file-carried roles instead of one skill's in-session check: intro **provides the data** (`gap-map.md` — each gap → filling chapter + callback-anchor) and summary **enforces the lock** (`summary-map.md`'s Callback entries, one per gap, + `check_summary.py`'s gap↔Callback bijection). Enforcement is structured-vs-structured (baton-vs-baton), never prose-vs-promise — whether the prose genuinely resolves the anchor is depth (author + eval); the mechanical gate is a write-time check, not a post-polish invariant.
_Avoid_: coherence guarantee (overclaims — the gate catches absence + 官僚 lapse, not depth), Intro-Discussion coherence (that's the article family's within-skill term)
...
**Overlap 清单 (method-overlap handoff)**:
The per-(shared-component × chapter-location) entries in `theory-map.md` recording where thesis-theory's lifted common material overlaps a body chapter's method section — each entry: which shared component ↔ theory §X ↔ chN §Y + a *suggested* disposition (e.g. 章内留 brief recap + cross-ref, or theory 收编/章简化). theory never edits sibling chapters (family spec aquarius #9 cut: theory is written last; "reorganizing" settled body chapters is rework); the **AUTHOR resolves each overlap manually**. This distinguishes it from `gap-map.md`'s callback promise: a promise's resolver is a downstream sibling skill (summary enforces via check), an overlap's resolver is the human — so no downstream skill ever enforces resolution; the overlap 段 is a handoff checklist, not a gate. A `disposition:` field the author fills later is optional audit-trail, never enforced.
_Avoid_: overlap resolution gate (overclaims — nothing gates it), dedup list (implies the skill removes the duplication itself)
